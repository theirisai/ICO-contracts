...
const balanceOf = require('../util').balanceOf;
const getParamFromTxEvent = require('../util').getParamFromTxEvent;
const assertThrowsAsynchronously = require('../util').assertThrowsAsynchronously;
...
contract('MultiSigWallet', (accounts) => {
    ...
    it('test execution after requirements changed', async () => {
        ...
        const transactionId = getParamFromTxEvent(
            await multisigInstance.submitTransaction(multisigInstance.address, 0, addOwnerData, {
                from: accounts[0]
            }),
            'transactionId',
            null,
        ...
        assert.deepEqual(
            await multisigInstance.getTransactionIds(0, 1, includePending, excludeExecuted), [transactionId]
        )
        ...
        const transactionId2 = getParamFromTxEvent(
            await multisigInstance.submitTransaction(multisigInstance.address, 0, updateRequirementData, {
                from: accounts[0]
            }),
            'transactionId',
            null,
        ...
        assert.deepEqual(
            await multisigInstance.getTransactionIds(0, 2, includePending, excludeExecuted), [transactionId, transactionId2]
        )
        ...
        assert.deepEqual(
            await multisigInstance.getTransactionIds(0, 1, excludePending, includeExecuted), [transactionId2]
        )
        ...
        assert.deepEqual(
            await multisigInstance.getTransactionIds(0, 2, excludePending, includeExecuted), [transactionId, transactionId2]
        )
    })
})